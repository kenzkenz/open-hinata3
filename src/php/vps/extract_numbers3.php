...
// **PDFをTIFFに変換**
//exec("pdftoppm -tiff -r 300 " . escapeshellarg($filePath) . " " . escapeshellarg($outputBase));
//// **変換後のファイル名 (`-1.png` を削除)**
...
//
//
//// ImageMagickで黒文字部分を赤色に変換
//$redTiffPath = pathinfo($finalOutputTif, PATHINFO_DIRNAME) . '/' . pathinfo($finalOutputTif, PATHINFO_FILENAME) . '_red.tif';
//
//$convertCmd = "convert " . escapeshellarg($finalOutputTif) . " -alpha set -channel RGB -colorspace Gray -negate " .
//    "-fill red -opaque black -alpha copy " . // 黒を赤に変更
//    "-transparent white " . // 白を透明にする
//    escapeshellarg($redTiffPath);
//
//
