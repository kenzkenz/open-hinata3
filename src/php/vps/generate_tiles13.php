...
// 白色を黒色に変換する処理（gdal_translateを使用）
sendSSE(["log" => "白色を黒色に変換する処理を開始します"]);
$whiteToBlackOutputPath = "/tmp/" . $fileName . "_white_to_black.tif";
$transparentCommand = "gdal_translate -mask 255,255,255 -a_nodata 0 " . escapeshellarg($outputFilePath) . " " . escapeshellarg($whiteToBlackOutputPath);
exec($transparentCommand, $transparentOutput, $transparentReturnVar);
if ($transparentReturnVar !== 0) {
    $error = ["error" => "透過処理失敗", "details" => implode("\n", $transparentOutput)];
    logMessage("Transparency processing failed: " . json_encode($error, JSON_UNESCAPED_UNICODE));
    sendSSE($error, "error");
    exit;
}
logMessage("Transparency processing completed: $whiteToBlackOutputPath");
sendSSE(["log" => "白色から黒色への変換が完了しました"]);
...
// クリーンアップ
function deleteSourceAndTempFiles($filePath, $tempOutputPath = null, $whiteToBlackOutputPath = null) {
...
deleteSourceAndTempFiles($filePath, $isJpeg ? $tempOutputPath : null, $whiteToBlackOutputPath);
