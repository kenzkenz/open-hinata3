...
if (!$requestData || !isset($requestData['geojson'])) {
    echo json_encode(['error' => 'Invalid request: Missing geojson key']);
    exit;
}

// GeoJSONを抽出
$geojsonData = $requestData['geojson'];
$scaleFactor = isset($requestData['scale']) ? floatval($requestData['scale']) : 1000;
...
if (file_put_contents($geojsonFile, json_encode($geojsonData)) === false) {
...
$command = "EZDXF_DISABLE_CACHE=1 $pythonPath $pythonScript " .
