...
$jpegPath = $uploadDir . "thumbnail-" . $fileBaseName . ".jpg";
...
$gdalCommand = "gdal_translate -of JPEG -co PAM=NO -outsize 100 0 " . escapeshellarg($tiffPath) . " " . escapeshellarg($jpegPath);
exec($gdalCommand . " 2>&1", $output, $returnVar);

// `.aux.xml` を削除
$auxXmlFile = $jpegPath . ".aux.xml";
...
// **JPEG 変換の結果をチェック**
if ($returnVar !== 0) {
    echo json_encode(["error" => "JPEG サムネイルの作成に失敗しました", "output" => $output]);
    exit;
}
