...
if (!in_array($fileExt, ["kmz", "kml"])) {
    echo json_encode(["error" => "許可されていないファイル形式です (KMZ, KML のみ許可)"]);
    exit;
}

// 一意のファイル名を作成
$fileBaseName = uniqid();
$kmzPath = $uploadDir . $fileBaseName . "." . $fileExt;

// ファイルの保存
if (!move_uploaded_file($_FILES["file"]["tmp_name"], $kmzPath)) {
...
echo json_encode([
    "success" => true,
    "kmzPath" => $kmzPath,
    "kmzName" => $originalFileName,  // 元のファイル名を追加
    "dir" => $subDir
]);
