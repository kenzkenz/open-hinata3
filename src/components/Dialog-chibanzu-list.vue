...
export default {
  ...
  computed: {
    ...
    s_dialogs () {
      return this.$store.state.dialogs.chibanzuListDialog
    },
  ...
  methods: {
    ...
    blueAndGraySelect (color) {
      ...
      async function fetchUserData(color) {
        try {
          const response = await axios.get('https://kenzkenz.xsrv.jp/open-hinata3/php/userBlueAndGraySelect.php', {
            params: { color: color}
          });
          if (response.data.error) {
            console.error('エラー:', response.data.error);
            ...
            if (color === 'blue') {
              vm.jsonDataBlue = response.data.result
            } else {
              vm.jsonDataGray = response.data.result
            }
          }
        } catch (error) {
          console.error('通信エラー:', error);
    ...
    blueAndGraySearch (color) {
      ...
      let name
      if (color === 'blue') {
        name = this.blueSearchText
      } else {
        name = this.graySearchText
      }
      async function fetchUserData(color) {
        try {
          const response = await axios.get('https://kenzkenz.xsrv.jp/open-hinata3/php/userBulueAndGreenPmtilesSearch.php', {
            params: {
              color: color,
              name: name
            }
