...
export default {
  ...
  props: {
    ...
    /**
     * ★ タイトル文字列をそのまま HTML として描画するか。
     *  既存互換のためデフォルト false（従来通りテキスト）
     *  NOTE: 信頼できない文字列を true にする場合はサニタイズ推奨。
     */
  ...
  methods: {
    /**
     * Close ハンドラ（ネイティブイベントを受け取り親へエミット）
...
<!--      &lt;!&ndash; Close ボタン（イベントを引数で受け取り親へエミット） &ndash;&gt;-->
<!--      <button class="close-btn" @click.stop="close($event)">×</button>-->
<!--    </div>-->
