    ...
    <v-card class="fw-card" :class="{ 'mobile-full': isMobile }">
      ...
      <div class="dialog-body">
        ...
        <div class="controls">
          ...
          <!-- ギャラリーから選ぶ（撮影中は非表示） -->
          <v-btn v-if="!shooting" size="small" variant="elevated" @click="openImage('gallery')">
            <v-icon size="18" class="mr-1">mdi-folder-image</v-icon>ギャラリーから選ぶ
          </v-btn>

          <!-- 非PWAまたはgetUserMedia不可の端末向けに、任意でカメラボタンを出す（撮影中は非表示） -->
          <v-btn
              v-if="!shooting && !canUseEmbeddedCamera"
              size="small"
              variant="text"
          ...
          <!-- やり直し（撮影中は非表示。プレビューがある時のみ表示） -->
          <v-btn v-if="!shooting && previewUrl" size="small" variant="text" @click="retake">
