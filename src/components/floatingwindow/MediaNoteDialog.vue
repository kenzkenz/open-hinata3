    ...
    <v-card class="fw-card" :class="{ 'mobile-full': isMobile }">
      ...
      <div class="d-flex align-center justify-between px-3 py-2 border-b">
        <div class="d-flex align-center" style="gap:8px">
          ...
          <div class="text-subtitle-1">画像とノート（任意）</div>
        </div>
      </div>

      <!-- 本文（ここだけがスクロール領域） -->
      <div class="dialog-body">
        <!-- プレビュー：縦長にも対応（contain） -->
        ...
        <!-- 操作ボタン群（PWAではライブ撮影をメインに、メニューは撤去） -->
        <div class="controls">
          <!-- PWA + getUserMedia可 + 非iOS：ライブ撮影メイン -->
          <template v-if="canUseEmbeddedCamera">
            <v-btn size="small" variant="tonal" color="primary" @click="onEmbeddedCaptureClick">
          ...
          <!-- ギャラリーから選ぶ（常に表示） -->
          <v-btn size="small" variant="elevated" @click="openImage('gallery')">
            <v-icon size="18" class="mr-1">mdi-folder-image</v-icon>ギャラリー
          </v-btn>

          <!-- 非PWAまたはgetUserMedia不可の端末向けに、任意でカメラボタンを出す -->
          <v-btn
              v-if="!canUseEmbeddedCamera"
              size="small"
              variant="elevated"
              @click="openImage('camera')"
          >
            <v-icon size="18" class="mr-1">mdi-camera</v-icon>カメラで撮影
          </v-btn>

          <v-btn size="small" variant="text" :disabled="!previewUrl && !shooting" @click="retake">
            <v-icon size="18" class="mr-1">mdi-autorenew</v-icon>やり直し
          </v-btn>
        </div>
...
export default {
  ...
  computed: {
    ...
    canUseEmbeddedCamera(){
      // PWA(standalone) かつ getUserMedia 使用可、かつ iOSでない（機種差大のため除外）
  ...
  methods: {
    ...
    async onSave({ imageMaxBytes = 1 * 1024 * 1024 } = {}) {
      ...
      this.saving = true;
      try {
        const kind = this.previewKind || 'none';  // 'image' | 'none'
        ...
        let media_processing = 0; // 動画機能撤去

        if (outFile && kind === 'image') {
          const fd1 = new FormData();
          fd1.append('dir', this.userId);
          fd1.append('kind', 'image');
          fd1.append('media', outFile, outFile.name || 'media.jpg');
        ...
        fd2.append('media_kind', kind);                   // 'none' | 'image'
        fd2.append('media_path', media_path);
        fd2.append('abs_path',   abs_path);
        fd2.append('media_size', media_size);
        fd2.append('media_processing', String(media_processing));
    ...
    openImage(mode){ // 'camera' | 'gallery'
      ...
      // PWAのときはカメラを強制的にライブへ振る
      if (mode === 'camera' && this.canUseEmbeddedCamera) {
        this.startCameraEmbedded();
        return;
      }
...
<!--      @update:modelValue="onUpdate"-->
<!--  >-->
<!--    &lt;!&ndash; 3分割グリッド化＋モバイルは実画面高相当 &ndash;&gt;-->
<!--    <v-card class="fw-card" :class="{ 'mobile-full': isMobile }">-->
<!--      &lt;!&ndash; ヘッダー &ndash;&gt;-->
<!--      <div class="d-flex align-center justify-between px-3 py-2 border-b">-->
<!--        <div class="d-flex align-center" style="gap:8px">-->
...
<!--      &lt;!&ndash; 本文（ここだけがスクロール領域） &ndash;&gt;-->
<!--      <div class="dialog-body">-->
<!--        &lt;!&ndash; プレビュー：縦長にも対応（contain）／動画にも対応 &ndash;&gt;-->
<!--        <div class="preview-wrap">-->
<!--          <div v-if="shooting" class="preview-box">-->
<!--            &lt;!&ndash; PWA埋め込みカメラ（getUserMedia） &ndash;&gt;-->
<!--            <video ref="videoEl" class="preview-video" autoplay playsinline muted></video>-->
<!--          </div>-->
<!--          <div v-else-if="previewUrl" class="preview-box">-->
...
<!--        &lt;!&ndash; 操作ボタン群 &ndash;&gt;-->
<!--        <div class="controls">-->
<!--          &lt;!&ndash; 分岐付きメインボタン（撮影 / ギャラリー） &ndash;&gt;-->
...
<!--          &lt;!&ndash; PWA環境での埋め込み撮影（安定・低メモリ） &ndash;&gt;-->
<!--          <v-btn-->
<!--              v-if="canUseEmbeddedCamera"-->
<!--              size="small"-->
<!--              variant="tonal"-->
...
<!--      &lt;!&ndash; 隠しファイル入力：用途別に用意 &ndash;&gt;-->
<!--      &lt;!&ndash; 画像: カメラ用（capture付き） &ndash;&gt;-->
<!--      <input-->
<!--          ref="fileInputImageCamera"-->
<!--          type="file"-->
<!--          accept="image/*"-->
...
<!--          @change="onFileChange('image', $event)"-->
<!--      />-->
<!--      &lt;!&ndash; 画像: ギャラリー/ファイル用（captureなし） &ndash;&gt;-->
<!--      <input-->
<!--          ref="fileInputImageGallery"-->
<!--          type="file"-->
<!--          accept="image/*"-->
