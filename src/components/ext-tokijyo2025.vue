...
export default {
  ...
  methods: {
    ...
    //       });
    //
    //       // 地番抽出：最後尾から漢字が現れるまで（例：5-57, 5, 5-5）
    //       let chibanWord = '';
    //       if (searchString.match(/\d+(-\d+)?$/)) { // 例: "5-57", "5", "5-5"
    //         chibanWord = searchString.match(/\d+(-\d+)?$/)[0]; // 地番部分
    //         searchString = searchString.replace(/\d+(-\d+)?$/, '').trim(); // 地番を除去
    //       }
    //
    //       const words = searchString.split(" ").filter(w => w);
    ...
    //       // 地番を除外したcombinedFields
    //       const combinedFields = ["concat", ["get", "市区町村名"], ["get", "大字名"], ["get", "丁目名"], ["get", "chibans"]];
    //
    //       if ((text.match(/"/g) || []).length >= 2) {
    //         // ダブルクォートで囲まれた場合の完全一致検索
    //         const filterConditions = words.map(word => [
    ...
    //           ["==", ["get", "大字名"], word.replace(/"/gi,'')],
    //           ["==", ["get", "丁目名"], word.replace(/"/gi,'')],
    //           // chibansに対して完全一致または部分一致
    //           [">=", ["index-of", word.replace(/"/gi,''), ["get", "chibans"]], 0]
    //         ]);
    //         matchCondition = ["all", ...filterConditions]
    //       } else {
    //         // 通常の部分一致検索
